...
v0.2ではDINOv2 CPU preflightで224を通過、448を不通過とし、DINOv2 224と古典2方式の比較を事前登録・契約化しました。進捗は`v0.2.x`へ細分化し、現在は`v0.2.2`として正常参照20枚、正常校正881枚、opaque final-test 200枚の境界を固定済みです。画像decode、fit、scoring、label reveal、採用判断は未実施です。詳細は以下の英語本文を参照してください。
